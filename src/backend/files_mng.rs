...
pub fn file_list(directory: &str) -> Result<String, ServerError> {
    ...
    for entry in paths {
        ...
        let html_entry = if file_type == "directory" {
            format!(
                r#"<li class="{}"><div class="file-entry"><button onclick="location.href='/{}'">
                {}</button></div></li>"#,
                file_type,
                format!("{}/{}", directory.strip_prefix("files/").unwrap_or(directory), file_name),
                file_name
            )
        } else {
            format!(
                r#"<li class="{}"><div class="file-entry"><span>{}</span><a href="/download/{}" 
                class="custom-button">Download</a></div></li>"#,
                file_type,
                file_name,
                format!("{}/{}", directory.strip_prefix("files/").unwrap_or(directory), file_name)
            )
